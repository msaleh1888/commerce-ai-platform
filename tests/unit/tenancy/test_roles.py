...
def test_administrator_can_manage_members_and_catalog() -> None:
    ...
    assert "tenant.member:manage" in capabilities
    assert "catalog.approval:execute" in capabilities


def test_lower_privilege_roles_cannot_approve_catalog_mutations() -> None:
    assert role_allows(Role.CATALOG_MANAGER, "catalog.approval:execute")
    assert not role_allows(Role.MERCHANDISER, "catalog.approval:execute")
    assert not role_allows(Role.AI_ENGINEER, "catalog.approval:execute")
    assert not role_allows(Role.VIEWER, "catalog.approval:execute")
